...
test('Motherboard: add devices', t => {
  ...
  m.addDevice(new MockDev());
  t.equal(m.devices.length, 1, 'one device added');
  t.equal(m.devices[0].CONST, 0xF0001123, 'device constant set');
  t.equal(m.get(0xF0001001), 1, 'device version is ok');
  t.end();
});
...
test('Motherboard: devices addresses', t => {
  ...
  m.addDevice(new MockDev());
  m.addDevice(new MockDev());
  t.equal(m.get32(0xF0000000), 0xF0001000, 'device 0 address');
  t.equal(m.get32(0xF0000004), 0xF0003000, 'device 1 address');
  t.end();
});
...
test('Motherboard: mmu access', t => {
  ...
  t.pass(m._mmu, '_mmu is set');
  m.set(0x5, 0x42);
  t.equal(m.get(0x5), 0x42, 'mmu is working');
  t.end();
});
...
test('Motherboard: memory map', t => {
  ...
  t.equal(memoryMap.length, expectedMemoryMap.length, '# of entries in memoryMap is correct');
  for(let i = 0; i < memoryMap.length; ++i) {
    t.equal(memoryMap[i].addr, expectedMemoryMap[i].addr, 'memoryMap.address is correct (record ' + i + ')');
    t.equal(memoryMap[i].deviceType, expectedMemoryMap[i].deviceType, 'memoryMap.deviceType is correct (record ' + i + ')');
    t.equal(memoryMap[i].size, expectedMemoryMap[i].size, 'memoryMap.size is correct (record ' + i + ')');
  }
  t.end();
});
...
test('Motherboard: unauthorized read', t => {
  ...
  m.addDevice(new MockMMU());
  m.get(0xFF000000);
  t.equal(m.get(m.MB_ERR), m.MB_ERR_UNAUTH_READ, 'MB_ERR_UNAUTH_WRITE is set');
  t.end();
});
...
test('Motherboard: unauthorized write', t => {
  ...
  m.addDevice(new MockMMU());
  m.set(0xFF000000, 1);
  t.equal(m.get(m.MB_ERR), m.MB_ERR_UNAUTH_WRITE, 'MB_ERR_UNAUTH_WRITE is set');
  t.end();
});
